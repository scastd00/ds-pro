...
char *getRequest(int delegateSocket) {
	...
	if (request == (char *) 0) {
		...
		n_bytes = recv(delegateSocket, request, MAX_REQ_SIZE, 0);

		if (n_bytes <= 0) {
//			perror("Error upon recv() on a delegateSocket. Worker thread exiting.\n");
			printf("Error upon recv() on a delegateSocket (%d). Worker thread exiting.\n", delegateSocket);
			fflush(stdout);
...
//		fflush(stdout);
		...
		//---------------
		// The result is sent back after the string RESP_MULT_2 in Network Byte Order:
